...
import { Invitation, InvitationStatus } from '@/types/organization'
...
type InviteInputValues = z.input<typeof inviteSchema>

/**
 * Effective status of an invitation. Once the invited customer or support
 * agent accepts, the invitation must never be displayed as "pending" — the
 * acceptedAt timestamp is authoritative even if the status column lags.
 */
function effectiveInvitationStatus(
  invitation: Invitation
): InvitationStatus {
  return invitation.acceptedAt ? 'accepted' : invitation.status
}
...
export default function InvitationsPage() {
  ...
  const filteredInvitations = (data?.data ?? []).filter(
    (inv) =>
      statusFilter === 'all' ||
      effectiveInvitationStatus(inv) === statusFilter
  )
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<InviteInputValues, any, InviteFormValues>({
  ...
  const columns: ColumnDef<Invitation>[] = [
    ...
    },
    {
      ...
      header: 'Status',
      cell: ({ row }) => {
        const status = effectiveInvitationStatus(row.original)
        const statusColors = {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const isPending = effectiveInvitationStatus(invitation) === 'pending'
